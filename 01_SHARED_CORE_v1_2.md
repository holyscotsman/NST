...
> **Doc version:** v1.2 · **Owner:** CORE & INTEGRATION chat · **Last updated:** 2026-06-24 · **Contract status: FROZEN** (see §9a)
>
> **Change history**
> | Version | Date | Change |
> |---|---|---|
...
ctx = {
  ...
  rng,         // Rng               (§6): PER-GAME fork — core.rng.fork("game:"+id). next/int/range/pick/shuffle/fork
...
**Three keys are ADDITIVE** (present, but newer than the original 7-key list; rely on them only if you check for them):
...
> **Freeze guarantee:** the 7 guaranteed keys' names and method signatures will not change. The 3 additive keys may grow but won't shrink. `ctx.exit` is the single menu-return method — do not invent alternatives. `ctx.rng` is **already forked per game**, so games must NOT re-seed from globals (and must never call `Math.random`).
