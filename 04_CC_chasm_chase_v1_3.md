...
<!-- doc-version: v1.3 · baseline v1.0 = original file · MINOR (V1.1 CC#7: three one-shot power-gate buffs) -->
**Doc version:** v1.3 · **Owner:** single chat · **Edited:** 2026-07-12
...
- **~10% of cores are power cores:** on collect (after answering), grant a **temporary buff**:
  ...
  - *(v0.178.0, V1.1 CC#7 — three **one-shot** kinds join the pool, 8 total, equal draw weight:)*
  - **Overshield** (one-shot): a 6th pip above `SHIELDS_MAX` that absorbs exactly one crash whole
    (after ablative plating in precedence; grants the normal i-frame grace). Not refreshed by timers.
  - **Scholar** (one-shot, the study-time buff): the **next** gate's question window is **+50%** on
    top of the v0.126 base ×1.5 (20s → 30s stock → 45s scholared), then spent.
  - **Double-down** (one-shot): the **next** gate carries raised stakes — **+2 shields** on correct
    (capped at `SHIELDS_MAX`), **−3** on wrong, then spent. Boost charging is unchanged.
- Timed buffs are stack-safe (re-collecting refreshes duration); one-shots re-arm on re-collect.
  Every answer → `MasteryStore.record` + telemetry.
- HUD chips (one-shots): `⚨+1` overshield · `📖next` scholar · `⚖next` double-down.
...
- **v1.3 (2026-07-12)** — V1.1 CC#7: the power-core pool grows from five timed buffs to eight — three one-shot kinds added (**Overshield** extra pip absorbing one crash, **Scholar** +50% on the next gate's question window stacking with the v0.126 base ×1.5, **Double-down** next gate +2/−3). One-shots render as HUD chips; all consume-on-use semantics pinned in `cc-run.cjs`. No contract or bank change.
