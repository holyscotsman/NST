...
<!-- doc-version: v1.3 · baseline v1.0 = original file · MINOR (feel pass + planet beat + Session-3 systems documented; no contract change) -->
**Doc version:** v1.3 · **Owner:** single chat (was ARM) · **Edited:** 2026-06-28
...
**Death by timeout (`v0.50.0`, closed by trace).** `timeUp → choose(wrong) → damage → gameOver`: a timer expiry grades as a standard wrong answer; at 0 shields `damage()` triggers the GAME OVER panel. Eyes-on confirm lives in `BROWSER_QA.md` (QA-A5). No new harness — the flow is the ordinary wrong-answer path.
