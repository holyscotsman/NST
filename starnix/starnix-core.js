...
(function (global) {
   ...
   * =================================================================== */
  var MAX_BOX = 8;               // (v0.89.0, L5) ladder extended past 24h — see INTERVALS
  var MASTERED_BOX = 4;          // summary().masteredCount threshold
  // Review interval per box (ms). Box 0 is always due. (v0.89.0, L5) 3-day and 7-day
  // boxes added: mastered cards stop re-duing EVERY day forever, so the due queue stays a
  ...
  function makeMasteryStore(profile, opts) {
    ...
    // (v2.7.0) The Leitner record now lives in the SHARED store (shared/nst-mastery.js)
    // so an answer in WWTBANE or Practice Exams counts here too -- the thing the
    // README always promised. StarNix keeps everything that is StarNix's alone: XP,
    // ranks, achievements, dailies, the station meter, telemetry and pace stats.
    //
    // `profile.mastery` is now a LIVE VIEW of the shared records, so every existing
    // reader (achievements, readiness, the domain heatmap) keeps working untouched.
    // The profile no longer persists its own copy -- see makeLocalStorageProvider.
    var shared = (typeof global !== "undefined" && global.NSTMastery) ? global.NSTMastery : null;
    if (shared) {
      shared.migrateIfNeeded();            // fold legacy per-tool history in, once
      profile.mastery = shared.all();
    } else if (!profile.mastery) {
      profile.mastery = {};                // no shared module (bare harness): stay self-contained
    }
    var map = profile.mastery;

    function init(id) {
      return { id: id, seen: 0, correct: 0, incorrect: 0, streak: 0, box: 0, lastSeen: 0 };
    }
    return {
      record: function (id, correct, ctx) {
        var now = clock.now();
        var prevBox, wasDue, m;
        if (shared) {
          // StarNix's policy, passed to the shared engine: promote only when the
          // card is actually DUE, one box at a time.
          var res = shared.record(id, { correct: !!correct, gate: "due", step: 1, now: now });
          m = res.rec; prevBox = res.prevBox; wasDue = res.wasDue;
          map = profile.mastery = shared.all();
        } else {
          m = map[id] || (map[id] = init(id));
          prevBox = m.box;
          ...
          wasDue = !m.seen || (m.lastSeen + (INTERVALS[Math.min(m.box, INTERVALS.length - 1)] || 0)) <= now;
          m.seen++;
          ...
          if (correct) {
            m.correct++; m.streak++;
            if (m.box < MAX_BOX && wasDue) m.box++;
            if (!m.firstCorrectAt) m.firstCorrectAt = now;
          } else {
            m.incorrect++; m.streak = 0;
            if (m.box > 0) m.box--;            // modified Leitner: gentle decay
          }
        }
        ...
        addXP(profile, xpForAnswer(!!correct, prevBox, m.box));
        ...
        // signal Leitner boxes can't see; the aggregates are also the scheduler-tuning evidence base.
        ...
        // un-builds a module. Recomputed only when a box actually moved.
        if (m.box !== prevBox) {
          var mcS = 0; for (var mkS in map) { if (map[mkS] && map[mkS].box >= MASTERED_BOX) mcS++; }
          var poolNS = 0;
        ...
        if (dd && correct) {
          ...
          if (m.box > prevBox || (correct && wasDue && m.box === MAX_BOX)) dd.promotions++;
      ...
      dueList: function (now) {
        ...
        for (var k in map) {
          ...
          var m = map[k];
          var at = m.lastSeen + (INTERVALS[Math.min(m.box, INTERVALS.length - 1)] || 0);
      ...
      summary: function () {
        ...
        for (var k in map) {
          ...
          if (m.box >= MASTERED_BOX) masteredCount++;
        }
        return {
  ...
  var XP_AWARDS = {
    ...
    promotion: 15,       // Leitner box moved UP (correct answers only)
    mastered: 40,        // bonus when a question first crosses MASTERED_BOX
  ...
  function xpForAnswer(correct, prevBox, newBucket) {
    var n = correct ? XP_AWARDS.answerCorrect : XP_AWARDS.answerWrong;
    if (correct && newBucket > prevBox) {
      n += XP_AWARDS.promotion;
      if (newBucket === MASTERED_BOX) n += XP_AWARDS.mastered;   // first step INTO mastered
    }
    return n;
  }
  ...
  var ACH_LIST = [
    ...
    { id: "first-mastery",    icon: "✦",  xp: 50,  name: "First mastery",    desc: "Bring one question up to mastered.",
      check: function (s) { var m = s.profile && s.profile.mastery; if (!m) return false; for (var k in m) { if (m[k] && m[k].box >= MASTERED_BOX) return true; } return false; } },
    ...
    { id: "archivist",        icon: "🏛", xp: 200, name: "Archivist",        desc: "Master 25 questions.",
      check: function (s) { var m = s.profile && s.profile.mastery; if (!m) return false; var n = 0; for (var k in m) { if (m[k] && m[k].box >= MASTERED_BOX) { n++; if (n >= 25) return true; } } return false; } },
  ...
  function applyTuning(t) {
    ...
    if (typeof t.masteredBucket === "number" && t.masteredBucket >= 1 && t.masteredBucket <= MAX_BOX) {
      MASTERED_BOX = t.masteredBucket; okT = true;
    }
  ...
  function makeQuestionProvider(pack, mastery) {
    ...
    function classify(q, now) {
      ...
      var due = (m.lastSeen + (INTERVALS[Math.min(m.box, INTERVALS.length - 1)] || 0)) <= now;
    ...
    function candidates(opts) {
      ...
      function build(useDomain, useBand, useExcl, allowImagesOverride) {
        var out = [];
        for (var k = 0; k < all.length; k++) {
          var q = all[k];
          if (!(opts.allowImages || allowImagesOverride) && q.image) continue;   // exhibit Qs are exam-only (need a full-screen image)
      ...
      c = build(false, false, false); if (c.length) return { list: c, relaxed: true };  // allow excluded
      // (v2.5.3) LAST resort: every relaxation above keeps filtering exhibits out, so a
      // bank whose questions ALL carry an @image left this empty and next() threw —
      // crashing whichever game asked. The games already render an explicit
      // "exhibit served in error" notice for a leaked image question, so serving one
      // is strictly better than taking the game down.
      c = build(false, false, false, true); return { list: c, relaxed: true };
    }
    ...
    return {
      ...
      // Leitner interval/box math so the shell just renders numbers.
      stats: function () {
        var now = clock.now();
        ...
        for (var i = 0; i < all.length; i++) {
          ...
          if (m && m.seen) {
            ...
            if (m.box >= MASTERED_BOX) { p.mastered++; overall.mastered++; }
            var interval = INTERVALS[Math.min(m.box, INTERVALS.length - 1)] || 0;
  ...
  function makeLocalStorageProvider(opts) {
    ...
    function writeNow(p) {
      ...
      // (v2.7.0) `mastery` is a live view of the shared store (nst.mastery.v1) and is
      // persisted there, so it is omitted here rather than written twice -- a second
      // stale copy would only confuse migration and bloat the profile.
      try { storage.setItem(key, JSON.stringify(p, function (k, v) { return k === "mastery" && this === p ? undefined : v; })); provider.writeError = null; }
  ...
  StarNix.makeContext = function (gameId) {
    ...
    return {
      ...
      hangar: (c.profile && c.profile.kbbHangar) ? JSON.parse(JSON.stringify(c.profile.kbbHangar)) : null,   // (v0.201.0, KBB#10)
  ...
  StarNix.tuning = {
    ...
    current: function () {
      return { intervals: INTERVALS.slice(),
        w: { due: W.due, "new": W["new"], reinforce: W.reinforce, epsilon: W.epsilon },
        masteredBucket: MASTERED_BOX, timer: Object.assign({}, TIMER) };
    },
    apply: applyTuning,
    set: function (t) {
  ...
  StarNix._internal = {
    ...
    constants: { MAX_BOX: MAX_BOX, MASTERED_BOX: MASTERED_BOX, INTERVALS: INTERVALS, W: W }
  };
