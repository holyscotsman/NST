...
(function (global) {
  ...
  Shell.prototype.boot = function (root, opts) {
    ...
    return StarNix.initCore(opts || {}).then(function () {
      ...
      // (v2.48.0) A bank that FAILED to fetch is said at boot, not after the intro.
      // The title screen offers Start, and playing the whole cinematic only to be told
      // the questions never arrived is the wrong order. A bank nobody has chosen is
      // untouched: that is not a fault, and the bridge is still worth looking around.
      if (typeof window !== "undefined" && window.STARNIX_BANK_ERROR) self.showNoBank();
      else self.showTitle();
      return self;
    });
  };
  ...
  Shell.prototype._buildStatsSummary = function (gridBox, listBox, opts) {
    ...
    for (var di = 0; di < n; di++) {
      ...
      row.appendChild(el("div", "sx-dom-name", (window.NSTDomainLabel ? window.NSTDomainLabel(d.domain) : d.domain)));
  ...
  Shell.prototype.showNoBank = function () {
    ...
    this.screen = "nobank";
    var self = this;
    // (v2.48.0) Two different situations shared one screen, and the one it described was
    // the one you could fix. "Pick a bank in the launcher" is sound advice to a player
    // who has not picked one; to a player whose bank is picked and whose file 404'd it
    // is a round trip to a launcher that will show that bank selected.
    var why = (typeof window !== "undefined" && window.STARNIX_BANK_ERROR) || null;
    var s = el("div", "sx-screen sx-panelwrap");
    var p = el("div", "sx-panel");
    p.appendChild(el("div", "sx-eyebrow", why ? "Question bank" : "No question bank"));
    p.appendChild(el("h2", "sx-h2", why ? "Couldn't load the question bank" : "No question bank loaded"));
    p.appendChild(el("div", "sx-note", why
      ? ((why === "manifest"
    ...
    if (why) {
      var bRetry = el("button", "sx-btn sx-btn-iris", "Try again");
      this._on(bRetry, "click", function () { window.location.reload(); });
      row.appendChild(bRetry);
      var bHome2 = el("button", "sx-btn sx-btn-ghost", "← Main menu");
      this._on(bHome2, "click", function () { window.location.href = "../"; });
      row.appendChild(bHome2);
    } else {
  ...
  Shell.prototype._injectShellCSS = function () {
    ...
    st.textContent = [
      ...
      "@media (max-width:600px){.sx-bridge-top{flex-direction:column;align-items:stretch;gap:10px;padding:14px 16px;}.sx-bridge-topright{flex-direction:column;align-items:stretch;gap:8px;min-width:0;}.sx-bridge .sx-menu-top{justify-content:flex-start;}.sx-bridge-dock{flex-direction:column;align-items:stretch;gap:10px;margin:14px calc(12px + env(safe-area-inset-right,0px)) calc(14px + env(safe-area-inset-bottom,0px)) calc(12px + env(safe-area-inset-left,0px));padding:12px;}.sx-bridge-dock .sx-daily{flex-direction:column;align-items:stretch;}.sx-bridge-dock .sx-daily-row{display:flex;align-items:center;justify-content:space-between;padding:6px 2px;}.sx-dock-cta,.sx-dock-continue{width:100%;}.sx-daily-claim{min-height:44px;padding:10px 14px;font-size:12px;}}",
      ...
      ".sx-dom-name{width:120px;color:var(--mid);}"   /* (v2.4.4) label comes from NSTDomainLabel */,
