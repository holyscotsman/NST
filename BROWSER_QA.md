...
- **QA-CC-BUFFS7 — the three one-shot buffs (added v0.178.0).** Fly CC through power gates
  ...
  next gate's timer visibly starts around 45s instead of 30s; DOUBLE-DOWN shows `⚖next` and
  the next gate's toast reads +2 on a correct / −3 on a wrong. WRONG: a chip that survives its
  own trigger, a 45s window on the second gate after, or double-down stacking with itself.
